...
class Manage:
    ...
    def InstructionFetch(self):
    	if self.stall in [1,3]:	# IF buffer cleared and no instruction pulled if there is a control hazard or program halted
    		return None
    	elif self.stall==2:
    ...
    def Execution(self, DecodeBuffer):
    	if DecodeBuffer==None: # step skipped in case of hazard/halt
    		return None
    	else:
    ...
    def Memory(self, ExecBuffer):
    	if ExecBuffer==None:	# step skipped in case of hazard/halt
    		return None
    	else:
    		...
    		return {"opcode": ExecBuffer["opcode"], "dest_ind": ExecBuffer["opcode"],
                "result": result}
    		
    
    def WriteBack(self, MemBuffer):
    	if MemBuffer==None: # Hazard/Halt
    		if self.FetchBuffer==None and self.DecodeBuffer=None and self.stall==3:
    			return -1 # -1 returned if halt instruction is "completed"
    		else
    			return 0 # # step skipped in case of hazard
    	else:
    		if opcode<=8: # writing to destination register
    			self.register_file[MemBuffer["dest_ind"]]["data"]=result
    			self.register_file[ExecBuffer["dest_ind"]]["busy"]-=1 # decrementing "busyness" of register to clear up data hazards
